...
class NhanVien ():
    ...
    def set_ma_nv(self, ma_nv): # Set giá trị cho mã nhân viên
        self.ma_nv=ma_nv
    def nhap_thong_tin(self,ds_nhanvien): # Phải truyền vào ds nhân viên vì khi sinh ra mã nhân viên tự động , python sẽ không biết thêm vào danh sách nào và việc tự sinh ra mã nhân viên sẽ bị lỗi
        print("Nhập thông tin :")
        self.ten_nv = input("Nhập tên nhân viên: ")
        while True: # Bắt người dùng nhập đúng định dạng số , tránh nhập chữ
            try:
    ...
    def xuat_thong_tin(self) -> str: #  ->str chỉ là type hint không ảnh hưởng tới kết quả trả về vì đã f string
        return f"| {self.ma_nv:^10} | {self.ten_nv:^20} | {self.luong_thang:^15,} | " # :^10  căn giữa và độ rộng 10 ký tự , căn trái :< , căn phải :>
