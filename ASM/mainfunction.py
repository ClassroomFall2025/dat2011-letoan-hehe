...
ds_nhanvien ={ # Tạo một dict chứa các list thông tin nhân viên thuộc loại Hành chính, Tiếp thị, Trưởng phòng 
        "Hành chính" :[],
        "Tiếp thị" :[],
        "Trưởng phòng": [] 
    }
hanh_chinh = 0 # Biến để tạo mã nhân viên
tiep_thi = 0
truong_phong = 0
...
def nhap_ds_nv():
    ...
    while True:
        ...
        try:
            ...
            continue #Bắt buộc người dùng nhập đúng số để phân loại kiểu nhân viên 
        
        if the_loai == 1:
            global hanh_chinh # Global ở đây là hàm và ta đã cho biến hành chính tăng thêm 1, nếu không để global thì hanhchinh sẽ biến thành biến cục bộ và sẽ bị lỗi chưa khai báo
            nhap_nhan_vien = NhanVien().nhap_thong_tin(ds_nhanvien["Hành chính"]) #Nhập thông tin vào key Hành chính của ds nhân viên 
...
def ghi_file():
    ...
    try:
        print("Đang ghi file...")
        with open(file= "danhsach_nhanvien"+ dinh_dang_file, mode="a", encoding= 'utf8') as file: #Ghi vào file danhsach nhanvien chế độ apend ( thêm vào cuối )           
...
def xuat_ds_theo_phong_ban(ds : dict): # truyền vào danh sách nhân viên 
    while True:
        while True:
            try:
        ...
        match loai:
            case 1:
                xuat_ds_theo_loai("Hành chính",ds["Hành chính"]) # truyền theo key hành chính và value là 1 list nhân viên thuộc loại Hành chính
            case 2:
